...
public class BuildMetadataFromXml {
  ...
  public static PhoneMetadataCollection buildPhoneMetadataCollection(String inputXmlFile,
    ...
    List<PhoneMetadata> collection = new ArrayList<PhoneMetadata>();
    ...
    metadataCollection.metadata = collection.toArray(new PhoneMetadata[collection.size()]);
  ...
  static boolean loadInternationalFormat(PhoneMetadata metadata,
    ...
    List<NumberFormat> formats = new ArrayList<NumberFormat>
        (Arrays.asList(metadata.intlNumberFormat));
    ...
    if (intlFormatPattern.getLength() > 1) {
      ...
      int leadingDigitsPatternSize = nationalFormat.leadingDigitsPattern.length;
      intlFormat.leadingDigitsPattern = new String[leadingDigitsPatternSize];
      for (int i = 0; i < leadingDigitsPatternSize; i++) {
        intlFormat.leadingDigitsPattern[i] = nationalFormat.leadingDigitsPattern[i];
      }
      intlFormat.nationalPrefixFormattingRule = nationalFormat.nationalPrefixFormattingRule;
      intlFormat.nationalPrefixOptionalWhenFormatting =
          nationalFormat.nationalPrefixOptionalWhenFormatting;
    } else {
    ...
    if (!intlFormat.format.equals("")) {
      formats.add(intlFormat);
      metadata.intlNumberFormat = formats.toArray(new NumberFormat[formats.size()]);
    }
  ...
  static void loadAvailableFormats(PhoneMetadata metadata,
    ...
    if (numOfFormatElements > 0) {
      for (int i = 0; i < numOfFormatElements; i++) {
        ...
        List<NumberFormat> formats = new ArrayList<NumberFormat>
            (Arrays.asList(metadata.numberFormat));
        ...
        formats.add(format);
        metadata.numberFormat = formats.toArray(new NumberFormat[formats.size()]);
  ...
  static void setLeadingDigitsPatterns(Element numberFormatElement, NumberFormat format) {
    ...
    if (numOfLeadingDigitsPatterns > 0) {
      List<String> pattern = new ArrayList<String>(Arrays.asList(format.leadingDigitsPattern));
      for (int i = 0; i < numOfLeadingDigitsPatterns; i++) {
        pattern.add(
            validateRE((leadingDigitsPatternNodes.item(i)).getFirstChild().getNodeValue(), true));
      }
      format.leadingDigitsPattern = pattern.toArray(new String[pattern.size()]);
    }
  }
