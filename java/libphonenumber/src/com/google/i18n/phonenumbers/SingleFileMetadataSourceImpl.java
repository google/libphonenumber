...
final class SingleFileMetadataSourceImpl implements MetadataSource {
  ...
  public PhoneMetadata getMetadataForNonGeographicalRegion(int countryCallingCode) {
    ...
    // code, i.e. RegionCode.UN001. If this is true of the given country calling code, then we will
    // return null here. If not for the atomic reference, such as if we were loading in multiple
    // stages, we would check that the passed in country calling code were indeed non-geographical
    // to avoid loading costs for a null result. Here though we do not check this since the entire
    // data must be loaded anyway if any of it is needed at some point in the life cycle of this
    // class.
    return MetadataManager.getSingleFileMetadataMaps(phoneNumberMetadataRef,
