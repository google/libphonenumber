...
/**
 @brief Gets a PhoneNumberToCarrierMapper instance to carry out international carrier lookup.
 <p> The PhoneNumberToCarrierMapper is implemented as a singleton. Therefore, calling this method multiple times will only result in one instance being created.
 @return a PhoneNumberToCarrierMapper instance
 */
...
/**
 ...
 As per #getNameForValidNumber(PhoneNumber,Locale) but explicitly checks the validity of the number passed in.
...
/**
 @brief Gets the name of the carrier for the given phone number only when it is 'safe' to display to users.
 A carrier name is considered safe if the number is valid and for a region that doesn't support <a href="http://en.wikipedia.org/wiki/Mobile_number_portability">mobile number portability</a>.
 @param number the phone number for which we want to get a carrier name
 @param languageCode the language code in which the name should be written
...
J2OBJC_EMPTY_STATIC_INIT(ComGoogleI18nPhonenumbersPhoneNumberToCarrierMapper)
