 ...
 private:
  ...
  class Repr {  // A byte-string.
   public:
    ...
    Repr()
        : data_(nullptr), size_(0), capacity_(0), ours_(true),
          utf8_was_valid_(true) {}
    ~Repr() {
...
typedef pair<UnicodeText::const_iterator, UnicodeText::const_iterator>
    UnicodeTextRange;
...
inline UnicodeText UTF8ToUnicodeText(const string &utf_string, bool do_copy) {
  return UTF8ToUnicodeText(utf_string.data(),
                           static_cast<int>(utf_string.size()), do_copy);
}
